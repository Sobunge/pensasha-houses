...
public class AuthController {
        ...
        public ResponseEntity<?> addUser(@Valid @RequestBody CreateUserDTO userDTO, BindingResult result) {
                        ...
                        return ResponseEntity.status(HttpStatus.CONFLICT)
                                        .body(EntityModel.of(userDTO,
                                                        linkTo(methodOn(UserController.class)
                                                                        .gettingUser(userDTO.getIdNumber())) // Self-link
                                                                                                             // to the
                                                                                                             // user
                                                                        .withSelfRel()));
                }
                ...
                // Wrap the saved user in a HATEOAS response model, including links to the user
                // and all users
                EntityModel<GetUserDTO> userModel = EntityModel.of(savedUser,
                                linkTo(methodOn(UserController.class).gettingUser(savedUser.getIdNumber())) // Self-link
                                                                                                            // to the
                                                                                                            // saved
                                                                                                            // user
                                                .withSelfRel(),
                                linkTo(methodOn(UserController.class).getAllUsers(1, 10)).withRel("all-users")); // Link
                                                                                                                 // to
                                                                                                                 // all
                                                                                                                 // users
        ...
        public ResponseEntity<Map<String, Object>> login(@Valid @RequestBody LoginRequestDTO request,
                ...
                List<String> roles = userDetails.getAuthorities().stream()
                                .map(grantedAuthority -> grantedAuthority.getAuthority()) // Map authorities to string
                                                                                          // roles
                                .collect(Collectors.toList());
