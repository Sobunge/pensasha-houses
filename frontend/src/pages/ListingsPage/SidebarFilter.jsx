...
const SidebarFilter = ({ onFilter }) => {
  ...
  const handleClearFilters = () => {
    setCategory(""); setType(""); setLocation(""); setBedrooms(""); setBathrooms("");
    setPriceRange([10000, 1000000]); setSizeRange([0, 1000]);
    onFilter?.({});
  };
    ...
    >
      ...
      <Box>
        ...
        <Collapse in={open}>
          ...
          <FormControl fullWidth size="small" sx={{ mb: 1 }}>
            ...
            <Select
              value={category}
              onChange={(e) => { setCategory(e.target.value); setType(""); setBedrooms(""); setBathrooms(""); }}
              label="Category"
              sx={{ ...fontStyle }}
            >
              <MenuItem value=""><em>Select Category</em></MenuItem>
              {propertyCategories.map((option) => (
                <MenuItem key={option} value={option} sx={fontStyle}>{option}</MenuItem>
              ))}
            </Select>
          </FormControl>
            ...
            <>
              <FormControl fullWidth size="small" sx={{ mb: 1 }}>
                ...
                >
                  ...
                  {bedOptions.map((num) => <MenuItem key={num} value={num} sx={fontStyle}>{num}+</MenuItem>)}
                </Select>
              </FormControl>
              ...
              <FormControl fullWidth size="small" sx={{ mb: 1 }}>
                ...
                >
                  <MenuItem value=""><em>Any</em></MenuItem>
                  {bathOptions.map((num) => <MenuItem key={num} value={num} sx={fontStyle}>{num}+</MenuItem>)}
                </Select>
              </FormControl>
            </>
          )}
      ...
      <Box>
        ...
        <Button
          ...
          sx={{ ...fontStyle, textTransform: "none", py: 0.7, borderRadius: "8px" }}
        >
          Clear
        </Button>
      </Box>
